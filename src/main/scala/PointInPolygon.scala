...
/**
 * Source: https://github.com/smagnacco/scala-point-in-polygon.git
 *
...
case class Polygon(points: List[GeoPoint]) {
  def corners: Int = points.size
  def horizontalCoordinates: List[Double] = points map (_.latitude)
  def verticalCoordinates: List[Double] = points map (_.longitude)
}
